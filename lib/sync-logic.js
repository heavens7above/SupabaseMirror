...
function mapSheetsToSupabase(row, headers) {
  ...
  // 2. PRIMARY MAPPING (With Type Filtering)
  headers.forEach((header, index) => {
    ...
    if (val && key.endsWith('_id') && key !== 'id') {
      if (!UUID_REGEX.test(String(val).trim())) val = null;
    }

    if (val && (key.endsWith('_at') || key === 'timestamp')) {
      if (!ISO_DATE_REGEX.test(String(val).trim()) && isNaN(Date.parse(String(val)))) val = null;
    }
    ...
    if (key.startsWith('is_')) {
      if (typeof val === 'string') val = val.toLowerCase() === 'true' || val === '1';
  ...
  // Find valid Name (Not a UUID, Not a Date, Not a Number, Not "sheets")
  if (!record.name) {
    const nameCandidate = row.find(v => {
      if (!v || typeof v !== 'string' || v.length < 2) return false;
      if (v.toLowerCase() === 'sheets' || v.toLowerCase() === 'supabase') return false;
  ...
  // Find valid Price (The first column that is a valid number > 0)
  if (!record.price && record.price !== 0) {
    const priceCandidate = row.find(v => v && !isNaN(Number(v)) && Number(v) > 0);
    if (priceCandidate) {
