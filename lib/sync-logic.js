...
function calculateFingerprint(record, includeKeys = null) {
  ...
  keys.sort().forEach(key => {
    // Skip internal metadata fields unless they are EXPLICITLY requested via sheet headers
    if (!includeKeys && ['synced_at', 'created_at', 'source', 'updated_at'].includes(key)) {
      return;
    }
    
    // If includeKeys is provided, we only hash what's in the sheet.
    // If NOT provided, we hash everything EXCEPT the metadata above.
