...
function mapSupabaseToSheets(record, headers) {
  ...
  // Map values exactly to the header positions (Case-insensitive)
  return headers.map(header => {
    if (!header || String(header).trim() === "") return "";
    const key = String(header).trim().toLowerCase();
    
    // Look for the value case-insensitively in the record
    let val = record[key];

    // Some fields in the payload might retain original casing
    if (val === undefined) {
       const originalKeyMatch = Object.keys(record).find(k => k.toLowerCase() === key);
       if (originalKeyMatch) {
         val = record[originalKeyMatch];
       }
    }
...
function mapSheetsToSupabase(row, headers) {
  ...
  if (!rowId || !UUID_REGEX.test(String(rowId).trim())) {
    ...
    headers.forEach((header, index) => {
      ...
      if (val && (key.endsWith('_id') || key === 'id') && !UUID_REGEX.test(String(val).trim())) {
  ...
  const hasNameCol = headers.some(h => h && h.trim().toLowerCase() === 'name');
  if (!record.name && hasNameCol) {
  ...
  const hasPriceCol = headers.some(h => h && h.trim().toLowerCase() === 'price');
  if (!record.price && record.price !== 0 && hasPriceCol) {
  ...
  // IMPORTANT: Only run if category_id was NOT already mapped in the primary pass
  const hasCategoryCol = headers.some(h => h && h.trim().toLowerCase() === 'category_id');
  if ((record.category_id === undefined || record.category_id === null) && hasCategoryCol) {
...
function calculateFingerprint(record, includeKeys = null) {
  ...
  // CRITICAL FIX: clone `keys` before sorting so we don't mutate the global `headers` array
  [...keys].sort().forEach(key => {
    // ALWAYS skip internal metadata fields
    if (metadataFields.includes(key.toLowerCase())) {
      return;
    }
      
      // Case-insensitive lookup for stable fingerprints across sources (Sheets vs DB)
      const val = record[key] !== undefined ? record[key] : record[key.toLowerCase()];
      
      if (val !== undefined && val !== null) {
        // Coerce all business values to strings for stable comparison
        normalized[key.toLowerCase()] = typeof val === 'object' 
          ? JSON.stringify(val) 
          : String(val);
      } else if (val === null) {
        normalized[key.toLowerCase()] = null;
      }
    });
