...
/**
 * Maps Google Sheets row data to Supabase record with type safety.
...
function mapSheetsToSupabase(row, headers) {
  ...
  headers.forEach((header, index) => {
    ...
    // If the field is an ID field (id, *_id) and NOT a valid UUID, null it out
    // to prevent Supabase from crashing on "invalid input syntax for type uuid"
    if (val && (key === 'id' || key.endsWith('_id'))) {
      const stringVal = String(val).trim();
      if (!UUID_REGEX.test(stringVal)) {
        console.warn(`[TYPE SAFETY] Discarding invalid UUID for '${header}': "${stringVal}"`);
        
        // FUZZY DETECTION: If this was the primary 'id' column, search the row for a UUID
        if (key === 'id') {
          const foundUuid = row.find(v => v && UUID_REGEX.test(String(v).trim()));
          if (foundUuid) {
            console.log(`[TYPE SAFETY] Fuzzy Match! Found UUID elsewhere in row: ${foundUuid}`);
            val = foundUuid;
          } else {
            val = null;
          }
